...
def draw_ui():
    ...
    for btn, rect in buttons.items():
        pygame.draw.rect(screen, WHITE, rect, 2)  # Исправлено
        text = font.render(btn.capitalize(), True, BLACK)
...
def main():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1 and start_pos:
                    end_pos = event.pos
                    if tool == 'rectangle' and start_pos != end_pos:
        ...
        for shape in drawings:
            ...
            elif shape[0] == 'rectangle' and shape[1] != shape[2]:
                x1, y1 = shape[1]
                x2, y2 = shape[2]
                pygame.draw.rect(screen, shape[3], (x1, y1, x2 - x1, y2 - y1), 2)
            elif shape[0] == 'circle':
